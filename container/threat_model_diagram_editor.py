...
function initGraph(){
  try{
    // Ensure base paths are set before using mxGraph
    if(typeof mxClient !== 'undefined') { mxClient.basePath='/app/static'; mxClient.imageBasePath='/app/static/images'; }
...
(function(){
  // Use fetch+eval so the library runs in the same window scope as our code.
  // A dynamically appended <script> tag inside a sandboxed srcdoc iframe can
  // execute in an isolated context where globals aren't shared with our code.
  fetch('/app/static/mxClient.min.js')
    .then(function(r){
      if(!r.ok) throw new Error('HTTP '+r.status);
      return r.text();
    })
    .then(function(code){
      // Set required globals before evaluating
      window.mxBasePath='/app/static';
      window.mxImageBasePath='/app/static/images';
    ...
    .catch(function(e){
      document.getElementById('overlay').innerHTML='Could not load mxClient.min.js: '+e.message;
      document.getElementById('overlay').style.display='block';
      status('Library load failed: '+e.message);
    });
})();
function addShape(t){
  if(!graph)return;
